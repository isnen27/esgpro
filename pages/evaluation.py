...
if uploaded_data_file is not None and uploaded_model_file is not None:
    ...
    if target_column not in test_data.columns:
        ...
        st.stop() # Hentikan eksekusi jika kolom target tidak valid
    ...
    if not feature_columns:
        ...
        st.stop() # Hentikan eksekusi jika tidak ada fitur
    ...
    try:
        ...
        # Perhatikan: precision, recall, f1-score memerlukan average untuk multi-class
        st.write(f"**Presisi (weighted):** {precision_score(y_test, y_pred, average='weighted'):.4f}")
        st.write(f"**Recall (weighted):** {recall_score(y_test, y_pred, average='weighted'):.4f}")
        st.write(f"**F1-Score (weighted):** {f1_score(y_test, y_pred, average='weighted'):.4f}")
